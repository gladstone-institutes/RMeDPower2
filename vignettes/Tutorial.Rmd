...
library(devtools)
install_github('gladstone-institutes/RMeDPower2@july25Rev', build_vignettes=TRUE, force = TRUE)
```
...
knitr::include_graphics("~/Dropbox (Gladstone)/calcPower f1000_manuscript/Final_paper_materials/Revisions/Older Version_Figures/Figures/Figure1.png")
```
...
knitr::include_graphics("~/Dropbox (Gladstone)/calcPower f1000_manuscript/Final_paper_materials/Revisions/Older Version_Figures/Figures/Figure12.png")
```
...
plot_no <- 1
plots <- diagnose_res$plots_info$natural_scale$plots
captions <- diagnose_res$plots_info$natural_scale$captions

for (i in seq_along(plots)) {
  print(plots[[i]])
  cat(sprintf("\n\n\n**Figure %d:** %s\n\n", plot_no, names(plots)[i]))
  plot_no <- plot_no + 1
}
```
...
plots <- diagnose_res$plots_info$log_scale$plots
captions <- diagnose_res$plots_info$log_scale$captions
...
## for each experimental column
for(i in 1:length(diagnose_res$models)) {
  for(j in 1:length(design@experimental_columns)){
    print(paste0("Cooks distance estimates for model ", diagnose_res$models[i], " and experimental factor = ", design@experimental_columns[j]))
    print(diagnose_res$cooks_result[[i]][[j]])
  }
}
```
...
for(i in 1:length(diagnose_res$models)) {
  for(j in 1:length(design@experimental_columns)){
    print(paste0("Cooks distance estimates for model ", full_diag_res$models[i], " and experimental factor = ", design@experimental_columns[j]))
    print(full_diag_res$cooks_result[[i]][[j]])
  }
}
```
...
```{r}
full_res <- getEstimatesOfInterest(full_data_update, full_design, model)
print(full_res[[1]])
```
...
```{r}
estimate_res_w_interaction <- getEstimatesOfInterest(data, design, model)
...
plots <- diagnose_res$plots_info$natural_scale_wo_outliers$plots
captions <- diagnose_res$plots_info$natural_scale_wo_outliers$captions
...
```
Note, there are only two levels for the animal model variable. So the cook's distance estimates are not evaluated for this variable.
```{r}
##see which experimental columns correspond to which variable
print(design@experimental_columns)
## for each experimental column
for(i in 1:length(diagnose_res$models)) {
  for(j in 1:length(diagnose_res$cooks_result[[i]])){
    print(paste0("Cooks distance estimates for model ", diagnose_res$models[i], " and experimental factor = ", design@experimental_columns[j]))
    print(diagnose_res$cooks_result[[i]][[j]])
  }
}
...
```
```{r}
for (i in seq_along(power_plots)) {
  print(power_plots[[i]])
  cat(sprintf("\n\n\n**Figure %d:** %s\n\n", plot_no, captions[i]))
  plot_no <- plot_no + 1
}
...
```
Now, let us look at the same set of QC plots for models on the log-transformed responses without the identified outliers in the log-transformed model

```{r output-plots10, results='asis'}
plots <- diagnose_res$plots_info$log_scale_wo_outliers$plots
captions <- diagnose_res$plots_info$log_scale_wo_outliers$captions
...
plots <- diagnose_res_update$plots_info$natural_scale$plots
captions <- diagnose_res_update$plots_info$natural_scale$captions
...
plots <- diagnose_res_update$plots_info$log_scale$plots
captions <- diagnose_res_update$plots_info$log_scale$captions
...
knitr::include_graphics("~/Dropbox (Gladstone)/calcPower f1000_manuscript/Final_paper_materials/Revisions/Older Version_Figures/Figures/J-Kaye_M-Shin 20222503_Part5.png")
```
...
knitr::include_graphics("~/Dropbox (Gladstone)/calcPower f1000_manuscript/Final_paper_materials/Revisions/Older Version_Figures/Figures/J-Kaye_M-Shin 20222503_Part7.png")
```
...
knitr::include_graphics("~/Dropbox (Gladstone)/calcPower f1000_manuscript/Final_paper_materials/Revisions/Older Version_Figures/Figures/J-Kaye_M-Shin 20222503_Part9.png")
```
...
knitr::include_graphics("~/Dropbox (Gladstone)/calcPower f1000_manuscript/Final_paper_materials/Revisions/Older Version_Figures/Figures/J-Kaye_M-Shin 20222503_Part10.png")
```
...
knitr::include_graphics("~/Dropbox (Gladstone)/calcPower f1000_manuscript/Final_paper_materials/Revisions/Older Version_Figures/Figures/J-Kaye_M-Shin 20222503_Part8.png")
```
